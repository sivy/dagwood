...
def build_graph():
    ...
    for node, node_data in list(G.nodes(data=True)):
        ...
        penwidth = NODE_LINE_MIN + (
            NODE_LINE_MAX - NODE_LINE_MIN
        )/NODE_EDGES_CONST * (incoming + outgoing)
...
def homepage():
    ...
    return render_template(
        'index.html',
        nodes=nodes, classes=classes, data=request.args)
...
def graph():
    ...
    if request.method == 'POST':
        ...
        if 'node' in data:
            ...
            if title: # ignore from_id, beware
                from_id = dbutil.create_node(title, classes=class_)

            elif not from_id or not target:
